...
| `typescript-eslint`                 | TS 解析器 + TS 规则的整合包（v8+）         | 🔗 项目用 TS 才装它（装它即够）|
| `@typescript-eslint/parser`         | 让 ESLint 能解析 TS 语法                   | ⚪ 无需单独装（已被整合包包含）|
| `@typescript-eslint/eslint-plugin`  | TS 专用规则集                              | ⚪ 无需单独装（已被整合包包含）|
...
> 💡 **关于 `typescript-eslint` 整合包（v8 起的重要变化）**
>
> 从 **v8 起**，官方推出整合包 **`typescript-eslint`**，它把 `@typescript-eslint/parser` 和 `@typescript-eslint/eslint-plugin`（及其依赖）打包在内，**装它一个就够**，配置里也只需 `import tseslint from 'typescript-eslint'`。官方安装命令：
>
> ```bash
> npm install -D eslint @eslint/js typescript-eslint
> ```
>
> | 写法     | 旧版（v7 及更早）                                                              | 新版（v8+，推荐）                 |
> | -------- | ------------------------------------------------------------------------------ | --------------------------------- |
> | 安装     | 分别装 `@typescript-eslint/parser` + `@typescript-eslint/eslint-plugin`        | 只装 `typescript-eslint`          |
> | 配置导入 | 分别引入 parser 和 plugin                                                      | `import tseslint from 'typescript-eslint'` |
> | 版本对齐 | 要自己保证两个包版本一致                                                       | 整合包自动对齐                    |
>
> 所以**不要再单独安装**那两个包——它们本身独立可用，但用整合包更省事、不易装错版本（旧教程里写两条安装命令就是因为用的是 v7 及更早版本）。
