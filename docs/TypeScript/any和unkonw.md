...
#### `any` 类型关闭类型检查
特点：
1. **完全禁用类型检查：** 绕过 `TypeScript` 的**所有类型检查**，即编译器不会对 `any` 类型的变量进行类型检查。
...
#### `unknown` 类型开启类型检查
特点：
1. **类型检查：** `TypeScript` 会对 `unknown` 类型的变量进行**类型检查**，但在**使用时需要明确指定其具体类型**，先进行类型检查或类型断言。
...
- **谨慎使用 `any`：** `any` 虽然灵活但会失去 `TypeScript` 的类型安全性优势，只在必要时使用。
### 4. 使用场景举例
#### 使用场景 1：不确定数据来源 - `unknown`
```typescript
