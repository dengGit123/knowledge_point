...
```css
/* 四舍五入到最接近的倍数 */
width: round(var(--width), 50px);  /* 向 50 的倍数取整 */

/* 取余（结果带除数的符号） */
margin-left: mod(250px, 50px);  /* = 0px */
margin-left: mod(27px, 50px);   /* = 27px */

/* 取余（结果带被除数的符号） */
margin-left: rem(100px, 3px);  /* = 1px */
```
...
/* 主题色变体 */
:root {
  --primary: hsl(220 90% 50%);  /* 完整颜色值 */
}
.button {
  color: var(--primary);
  /* 相对颜色语法：基于 --primary 生成新颜色（较新语法，注意兼容性） */
...
filter: hue-rotate(180deg);  /* 互补色（旋转色相 180 度） */
```
