...
│margin │ ← 不算在 width 里
│ ┌─────────────────┐ │
│ │ ╔═══════════════╗ │ │ ← width = 200px 包含 padding、border
│ │ ║content  ║ │ │    （内容区实际为 150px）
│ │ ╚═══════════════╝ │ │
│ └─────────────────┘ │
└─────────────────┘
占用总宽度 = 200 + margin
...
| 模型 | width 含义 | 内容区宽度 | 元素总宽度（不含 margin） | 页面上占的总空间（含 margin） |
|-----|-----------|-----------|--------------------------|------------------------------|
| 标准模型 | 仅内容 | 200px | 200 + 40 + 10 = 250px | 250 + 20 = 270px |
| IE 模型 | 内容+padding+border | 200 - 40 - 10 = 150px | 200px | 200 + 20 = 220px |
...
/* content-box（默认） */
width = content
占用空间 = width + padding + border + margin
```
